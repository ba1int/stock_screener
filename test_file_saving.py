...
from stock_screener.utils.helpers import (
    save_news_data,
    save_selected_tickers,
    save_investment_summary,
)
from stock_screener.data.test_data import (
    SAMPLE_STOCKS,
    SAMPLE_NEWS_DATA,
    SAMPLE_ANALYSIS,
)
    ...
    try:
        ...
        if file_path:
            ...
            if Path(file_path).exists():
                logger.info(
                    f"File {file_path} exists and has size {Path(file_path).stat().st_size} bytes"
                )
            else:
